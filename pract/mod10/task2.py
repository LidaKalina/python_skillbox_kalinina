...
def is_valid_rgb(color):
    ...
    False
    """
    pattern = r'^rgb\((\d{1,2}%?|1?\d{1,2}%?|2[0-4]\d%?|25[0-5]%?), (\d{1,2}%?|1?\d{1,2}%?|2[0-4]\d%?|25[0-5]%?), (\d{1,2}%?|1?\d{1,2}%?|2[0-4]\d%?|25[0-5]%?)\)$'
...
def is_valid_hsl(color):
    ...
    False
    """
    pattern = r'^hsl\((\d{1,3}|[1-9]\d{0,2}),\s*(100%|[1-9]?\d%|0%),\s*(100%|[1-9]?\d%|0%)\)$'
    return bool(re.match(pattern,color))
